...
$response["mobile_Info"] = array();
 ...
 $mak=$obj->fetchByIdByStatusBymacbyfcm_token(1,"user_register_goverment","status","mobile",$mobile); 
...
if($check['success_date']){
			$endDate=(  date('d-m-Y', strtotime('+180day', strtotime($check['success_date'])))); 
			}else{
			    $endDate="";
			}
				$info["expiry_date"]=$endDate;
...
$response["error"] =true;
$response["success"] =0;
$response["msg"] = "Mobile Number & Password Does not Match or Already logged in from another device";
...
$response["error"] =true;
$response["success"] =0;
$response["msg"] = "Please Confirm Your Payment";
...
$response["error"] =true;
$response["success"] =0;
$response["msg"] = "Mobile number and Password not matched";
@array_push($response["mobile_Info"]);
echo json_encode($response);	
}
else
...
$response["error"] =true;
$response["success"] =0;
$response["msg"] = "Mobile Number & Password Does not Match";
@array_push($response["mobile_Info"]);
echo json_encode($response);
