...
include"menu.php"; ?> <!-- /. main header -->
			...
			$ud=implode(',',$us);
														  ...
														  elseif($row['user_type']==2) { echo "Executive Supervisor"; 
			...
			$ud=implode(',',$us);
														   ...
														   elseif($row['user_type']==4) { echo "Relationship Manager"; 
														  
														   $us="";
				    $u=explode(",",$row['member_id']);
				    foreach($u as $uu =>$value)
				    {
				      $user=$obj->fetchById($value,"referal_agent","id");
				         $us[]=$user['user_name'];
				    }
			$ud=implode(',',$us);
														  } 
														   elseif($row['user_type']==5) { echo "Block Supervisor"; 
			...
			$ud=implode(',',$us); }
			 elseif($row['user_type']==6)
				{echo "Free Access User"; 
					$us="";
				    $u=explode(",",$row['member_id']);
				    foreach($u as $uu =>$value)
				    {
				      $user=$obj->fetchById($value,"free_acess","id");
				         $us[]=$user['name'];
				    }
			$ud=implode(',',$us); }
															...
															elseif($row['user_type']==8) { echo "PRO"; 
														  
														   $us="";
				    $u=explode(",",$row['member_id']);
				    foreach($u as $uu =>$value)
				    {
				      $user=$obj->fetchById($value,"pro_register","id");
				         $us[]=$user['name'];
				    }
			$ud=implode(',',$us);
														  }
														  
						elseif($row['user_type']==9) { echo "E-Government "; 
