...
<div class="content-wrapper">
			 ...
			 ?></span></h2>
					...
					$rs=$obj->fetchDetailById(1,"user_register","status");
					if($rs)
					{
					echo mysqli_num_rows($rs);	
			 ...
			 ?></span> </h2>
				...
				$rowu41=0;
$rstt15=$obj->fetchDetailByIdByStatus(1,"user_register","class","pstatus",1);
...
$yy1=$dd['1'];
	...
	echo $rowu41;
			 ?></span> </h2>
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
                                <div class="statistic-box statistic-filled-2">
                                    <h2><span class="count-number"><?php $rowu41=0;
$rstt15=$obj->fetchDetailByIdByStatus(2,"user_register","class","pstatus",1);
//fetchDetailByIdByStatususer
if($rstt15)
{	$i=0;
while($row16=mysqli_fetch_assoc($rstt15))
...
$dd=explode("-",$date);
 $yy1=$dd['1'];
	...
	$to=0;
$rstt151=$obj->fetchDetailById(1,"user_register","status");
//fetchDetailByIdByStatususer
if($rstt151)
{	$i=0;
while($row161=mysqli_fetch_assoc($rstt151))
{	$i++;
  $id16=$row161['id'];
 $register_date16=$row161['register_date'];
 $dddd1=explode("-",$register_date16);
 $yy2=$dddd1['1'];
	$date=date("Y-m-d");
$dd=explode("-",$date);
...
{	$i++;
 $id16= $row16['id'];
		$rowu41=$rowu41+1;				//	$rs18=$obj->fetchDetailByIdByStatus($id16,"class_order","user_id","status",1);
													//fetchDetailByIdByStatususer
						//	if($rs18)
							//{	$i=0;
							//	while($row118=mysqli_fetch_assoc($rs18))
...
{	$i++;
 $id16= $row16['id'];
	$rowu42=$rowu42+1;					//	$rs18=$obj->fetchDetailByIdByStatus($id16,"class_order","user_id","status",1);
													//fetchDetailByIdByStatususer
							//if($rs18)
							//{	$i=0;
							//	while($row118=mysqli_fetch_assoc($rs18))
...
?></span> </h2>
						...
						$date=date("d-m-Y");
			$rowu41=0;
$rstt15=$obj->fetchDetailByIdByStatus(1,"user_register","status","payment_success_date",$date);
//fetchDetailByIdByStatususer
if($rstt15)
{	$i=0;
while($row16=mysqli_fetch_assoc($rstt15))
{	$i++;
 $id16= $row16['id'];
	...
	echo $rowu41;
			 ...
			 $date=date("d-m-Y");
			$tot=0;			
	$rs=$obj->fetchDetailByIdByStatus(1,"user_register","status","register_date",$date);
	//fetchDetailByIdByStatus($date,"user_register","payment_success_date","pstatus",0);
	if($rs)
	{	$i=0;
		while($row16=mysqli_fetch_assoc($rs))
		{	$i++;
